% RGB <-> YUV
% Implementación
% No queda claro por qué eligen calcular de a un sólo pixel.
% No aclaran el tipo de desempaquetado que utilizan, o cómo dejan el registro (XMM1) como está al comienzo. Al final explican que esta operación la repiten 4 veces, pero no aclaran igualmente por qué decidieron desempaquetar a dw.
% Hubiera quedado más claro si ponían los ejemplos de los registros que ejemplifican las máscaras justo debajo del texto que las nombra.
% No cuentan de qué algoritmo están contando la implementación, si de RGB a YUV o al revés
% En la explicación del valor repetido, escriben "El mismo no la correción del filtro" a qué se refieren?
% Está muy bien que hayan ilustrado los registros.
% Análisis preliminar
% Intenten ser lo más precisos posible con el detalle de la experimentación.
% En qué plataforma se corrió? 
% Dónde pusieron los mecanismos de medición? Al hacer una llamada al filtro? Al comenzar a ejecutar el tp2? Cuántas veces lo corrieron (exactamente)?
% No identificaron cuál algoritmo es cuál en los gráficos
% Experimentación
% La comparación con diferentes niveles de optimización en C tengo entendido que es parte del análisis preliminar.
% Son dos experimentos simples (cambiar un par de instrucciones) o uno complejo, que involucre más cambio de código.
% No identificaron cuál algoritmo es cuál en los gráficos
